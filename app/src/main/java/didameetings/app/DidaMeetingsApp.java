...
public class DidaMeetingsApp {
	...
	private boolean open(int mid) {
		...
		GenericResponseCollector<DidaMeetingsMain.OpenReply> open_collector = new GenericResponseCollector<DidaMeetingsMain.OpenReply>(
				open_responses, n_servers);

		for (int i = 0; i < n_servers; i++) {
			CollectorStreamObserver<DidaMeetingsMain.OpenReply> open_observer = new CollectorStreamObserver<DidaMeetingsMain.OpenReply>(
					open_collector);
	...
	private boolean add(int mid, int pid) {
		...
		GenericResponseCollector<DidaMeetingsMain.AddReply> add_collector = new GenericResponseCollector<DidaMeetingsMain.AddReply>(
				add_responses, n_servers);

		for (int i = 0; i < n_servers; i++) {
			CollectorStreamObserver<DidaMeetingsMain.AddReply> add_observer = new CollectorStreamObserver<DidaMeetingsMain.AddReply>(
					add_collector);
	...
	private boolean topic(int mid, int pid, int topic) {
		...
		GenericResponseCollector<DidaMeetingsMain.TopicReply> topic_collector = new GenericResponseCollector<DidaMeetingsMain.TopicReply>(
				topic_responses, n_servers);

		for (int i = 0; i < n_servers; i++) {
			CollectorStreamObserver<DidaMeetingsMain.TopicReply> topic_observer = new CollectorStreamObserver<DidaMeetingsMain.TopicReply>(
					topic_collector);
		...
		if (topic_responses.size() >= responses_needed) {
			...
			if (result) {
				System.out.println(
						"Meeting with id " + mid + " participant with id " + pid + " and topic " + topic + "\n");
			} else {
	...
	private boolean close(int mid) {
		...
		GenericResponseCollector<DidaMeetingsMain.CloseReply> close_collector = new GenericResponseCollector<DidaMeetingsMain.CloseReply>(
				close_responses, n_servers);

		for (int i = 0; i < n_servers; i++) {
			CollectorStreamObserver<DidaMeetingsMain.CloseReply> close_observer = new CollectorStreamObserver<DidaMeetingsMain.CloseReply>(
					close_collector);
	...
	private boolean show() {
		...
		GenericResponseCollector<DidaMeetingsMain.DumpReply> dump_collector = new GenericResponseCollector<DidaMeetingsMain.DumpReply>(
				dump_responses, n_servers);

		for (int i = 0; i < n_servers; i++) {
			CollectorStreamObserver<DidaMeetingsMain.DumpReply> dump_observer = new CollectorStreamObserver<DidaMeetingsMain.DumpReply>(
					dump_collector);
		...
		while (counter < loop_size) {
			...
			if (n_open < OPEN_THRESHOLD) {
				...
				if (action < (ACTION_PROBABILITY_RANGE * 0.25)) {
					...
					if (((n_open < OPEN_TARGET) && (action >= (ACTION_PROBABILITY_RANGE * 0.25)))
							|| ((n_open >= OPEN_TARGET) && (action < (ACTION_PROBABILITY_RANGE * 0.25)))) {
						int mid = rnd.nextInt(meetings_range);
					...
					if (parameter1 != null && parameter2 != null && parameter3 != null) {
						try {
							...
							if (topic(mid, pid, tid))
								System.out.println(
										"added topic " + tid + " to participant " + pid + " in meeting  " + mid);
							else
								System.out.println(
										"failed to add topic " + tid + " to participant " + pid + " in meeting " + mid);
						} catch (NumberFormatException e) {
					...
					if (parameter1 != null && parameter2 != null) {
						try {
							...
							if (add(mid, pid))
								...
								System.out.println(
										"failed to add  participant " + pid + " to meeting with id " + mid + ".");
						} catch (NumberFormatException e) {
	...
	public void main_loop(String[] args) throws Exception {
		...
		System.out.println("Client meetings_range = " + this.meetings_range + " participants_range = "
				+ this.participant_range + " sleep_range = " + this.sleep_range + " loop_size = " + this.loop_size);
