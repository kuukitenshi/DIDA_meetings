	...
	public synchronized void processEntry(int entry_number) {
		...
		while (next_entry.decided == false) {
			...
			if ((ballot > -1) && (request_record != null)
				...
				// send phase1
				DidaMeetingsPaxos.PhaseOneRequest.Builder phase_one_request_builder = DidaMeetingsPaxos.PhaseOneRequest
						.newBuilder();
				...
				// PhaseOneResponseProcessor(this.server_state.scheduler, low_ballot,
				// high_ballot);
				PhaseOneBogusProcessor phase_one_processor = new PhaseOneBogusProcessor(this.server_state.scheduler,
						low_ballot, high_ballot);

				ArrayList<DidaMeetingsPaxos.PhaseOneReply> phase_one_responses = new ArrayList<DidaMeetingsPaxos.PhaseOneReply>();
				GenericResponseCollector<DidaMeetingsPaxos.PhaseOneReply> phase_one_collector = new GenericResponseCollector<DidaMeetingsPaxos.PhaseOneReply>(
						phase_one_responses, n_acceptors, phase_one_processor);

				for (int i = 0; i < n_acceptors; i++) {
					CollectorStreamObserver<DidaMeetingsPaxos.PhaseOneReply> phase_one_observer = new CollectorStreamObserver<DidaMeetingsPaxos.PhaseOneReply>(
							phase_one_collector);
				...
				System.out.println("Paxos phase 1 ended with aborted = " + ballot_aborted + " and read ballot = "
						+ phase_one_processor.getValballot() + " and value " + phase_two_value);
				...
				if (ballot_aborted == false) {
					...
					// send phase2
					DidaMeetingsPaxos.PhaseTwoRequest.Builder phase_two_request = DidaMeetingsPaxos.PhaseTwoRequest
							.newBuilder();
					...
					GenericResponseCollector<DidaMeetingsPaxos.PhaseTwoReply> phase_two_collector = new GenericResponseCollector<DidaMeetingsPaxos.PhaseTwoReply>(
							phase_two_responses, n_acceptors, phase_two_processor);
					for (int i = 0; i < n_acceptors; i++) {
						CollectorStreamObserver<DidaMeetingsPaxos.PhaseTwoReply> phase_two_observer = new CollectorStreamObserver<DidaMeetingsPaxos.PhaseTwoReply>(
								phase_two_collector);
						this.server_state.async_stubs[acceptors.get(i)].phasetwo(phase_two_request.build(),
								phase_two_observer);
					}
		...
		System.out.println("Log entry with number " + this.next_log_entry + " has been decided with command id = "
		...
		// System.out.println("Going to process command with id = " +
		// next_entry.command_id);
		...
		switch (action) {
			case DidaMeetingsAction.OPEN:
				System.out.println("It is an open request with id = " + command.getMeetingId() + " and max = "
			...
			case DidaMeetingsAction.TOPIC:
				result = this.server_state.meeting_manager.setTopic(command.getMeetingId(), command.getParticipantId(),
						command.getTopicId());
				break;
		...
		// sending response
		System.out.println(
				"Setting response for command with id = " + next_entry.command_id + " with result = " + result);
